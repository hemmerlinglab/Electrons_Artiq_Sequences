...
class Zotino(EnvExperiment):
    ...
    def build(self):
        ...
        self.setattr_device('zotino0', 'zotino1', 'zotino2', 'zotino3', 'zotino4', 'zotino5', 'zotino6', 'zotino7', 'zotino8', 'zotino9')
#       self.setattr_device('scheduler')

#       self.setattr_argument('voltage', NumberValue(default=0,unit='V'))
...
#    def send(self, voltage):
	...
	self.zotino1.init()
        delay(200*us)
        self.zotino1.write_gain_mu(0, 65000)
        
        while(True):
            self.zotino1.write_dac_mu(0, 50000)
            self.zotino1.load()
            delay(10*us)
            self.zotino1.write_dac_mu(0, 1)
            self.zotino1.load()
            delay(20*us)

	self.zotino2.init()
        delay(200*us)
        self.zotino2.write_gain_mu(0, 65000)
        
        while(True):
            self.zotino2.write_dac_mu(0, 50000)
            self.zotino2.load()
            delay(10*us)
            self.zotino2.write_dac_mu(0, 1)
            self.zotino2.load()
            delay(20*us)

#        self.initialize_zotino()
#        self.send(self.voltage)
