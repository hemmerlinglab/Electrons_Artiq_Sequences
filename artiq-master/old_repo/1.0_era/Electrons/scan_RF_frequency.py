...
class Scan_RF_Frequency(EnvExperiment):
    ...
    def build(self):
        ...
        self.my_setattr('min_freq', NumberValue(default=1.5688,unit='GHz',scale=1,ndecimals=4,step=.0001))
        self.my_setattr('max_freq', NumberValue(default=1.5848,unit='GHz',scale=1,ndecimals=4,step=.0001))
    ...
    def prepare(self):
        ...
        self.scan_values = self.min_freq + np.arange(0, self.steps) * (self.max_freq - self.min_freq) / (self.steps-1)
    ...
    def run(self):
        ...
        for k in range(self.steps):
            ...
            print(str(k) + '/' + str(self.steps) + ': Setting RF frequency to ' + str(self.scan_values[k]) + 'GHz.')
            self.RF_driver.set_freq(self.scan_values[k]*1e+9)
