...
class Scan_Mesh_Voltage(EnvExperiment):
    ...
    def build(self):
        ...
        self.my_setattr('min_scan', NumberValue(default=0,unit='V',scale=1,ndecimals=1,step=1))
        self.my_setattr('max_scan', NumberValue(default=600,unit='V',scale=1,ndecimals=1,step=1))
        ...
        self.my_setattr('mesh_voltage', NumberValue(default=150,unit='V',min=0,max=600,scale=1,ndecimals=1,step=1))
    ...
    def run(self):
        ...
        for i in range(len(self.scan_values)):
            ...
            self.set_mesh_voltage( 1.0/200.0 * self.scan_values[i] )
            if i == 0:
                time.sleep(3)
            else:
                time.sleep(1)
